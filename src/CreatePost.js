...
export default function CreatePost({
  ...
  async function save() {
    try {
      const { name, description, location, image, type } = formState;
      if (!name || !description || !location || !image.name || !type) return;
      updateFormState(currentState => ({ ...currentState, saving: true }));
      const postId = uuid();
      const postInfo = { name, description, location, image: formState.image.name, id: postId, type };

      console.log('🚀 ~ file: CreatePost.js ~ line 47 ~ save ~ postInfo', postInfo)
